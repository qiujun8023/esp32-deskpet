...
static void task_auto_move(void *arg)
{
    ...
    while (1) {
        ...
        if ((esp_random() % prob) == 0) {
            ...
            int dur  = (mode == EYE_MODE_SOFT)
                       ? 50  + (esp_random() % 150)   // 50~200ms
...
void app_main(void)
{
    ...
    /* 显示任务固定到 Core 1，避免与 WiFi/网络争夺 Core 0 */
    xTaskCreatePinnedToCore(task_display,   "display",   4096, NULL, 5, NULL, 1);
    xTaskCreatePinnedToCore(task_auto_move, "auto_move", 2048, NULL, 3, NULL, 0);

    ESP_LOGI(TAG, "===== 系统启动完成 =====");
    ESP_LOGI(TAG, "连接 WiFi: 桌面机器人  访问 10.10.10.10");
}
