...
#define EYE_Y_DEF   ((H - EYE_H_DEF) / 2.0f)  // 14.0
...
void robo_eyes_set_mode(eye_mode_t m) {
    s_mode = m;
}
...
void robo_eyes_update(void) {
    ...
    if (s_mode != EYE_MODE_SLEEP) {
        if (t >= s_idle_next_us) {
            trigger_idle();
            int interval_ms = (s_mode == EYE_MODE_SOFT) ? rand_range(3000, 6000) : rand_range(1500, 4000);
    ...
    // 垂直居中补偿：高度变化时同步偏移 y，使眼睛从中心向上下均匀扩展/收缩
